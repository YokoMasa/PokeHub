...
import com.masalab.masato.githubfeed.http.ConnectionResult;
import com.masalab.masato.githubfeed.http.ResultBodyConverter;
...
class GitHubApiCallbackHandler {
    ...
    private static void setErrorInfoToGitHubApiResult(ConnectionResult result, GitHubApiResult gitHubApiResult) {
        if (result.isConnectionSuccessful) {
            ...
            gitHubApiResult.errorMessage = GitHubObjectMapper.getInstance().mapErrorMessage(result.getBodyString());
        } else {
